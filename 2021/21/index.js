...
export default function canCarry(capacity, trip) {
  const giftTimeline = new Map();
  let currentLoad = 0;

  trip.forEach(([gifts, pickup, delivery]) => {
    giftTimeline.set(pickup, (giftTimeline.get(pickup) || 0) + gifts);
    giftTimeline.set(delivery, (giftTimeline.get(delivery) || 0) - gifts);
  });

  const sortTimeline = [...giftTimeline.entries()].sort((a, b) => a[0] - b[0]);

  for (const [_, gifts] of sortTimeline) {
    currentLoad += gifts;
