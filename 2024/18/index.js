...
function findInAgenda(agenda, phone) {
  const find = [];
  const rows = agenda.split('\n');

  for (let i = 0; i < rows.length; i++) {
    const phoneRegex = /\+\d{1,2}-\d{3}-\d{3}-\d{3}/;
    const nameRegex = /<([^>]+)>/;
    const str = rows[i].trim();
    ...
    let address = str.replace(phoneNumber, '');
    address = address.replace(nameMatch[0], '');

    if (phoneNumber.includes(phone))
      find.push({ name, address: address.trim() });
  }

  return find.length === 1 ? find[0] : null;
}

const agenda = `+34-600-123-456 Calle Gran Via 12 <Juan Perez>
Plaza Mayor 45 Madrid 28013 <Maria Gomez> +34-600-987-654
<Carlos Ruiz> +1-800-555-019 Fifth Ave New York`;
